...
function writeTextToAddress(rom, inAddress, inLength, inText, nullPad = false)
{
	...
	if( inLength > 0 ) //clear room for the text
	{
		...
		{
			//changing to always 0x00 broke symbol hash
			//added optional parameter for things like item names that need to be null teriminated
			nullPad ? rom[inAddress + i] = 0x00 : rom[inAddress + i] = TEXT_MAPPING[" "];
		}
	}
...
function convertTextToHex(inText)
{
	var hexArray = [];
	for (var i = 0; i < inText.length; ++i)
	{
		hexArray[i] = TEXT_MAPPING[inText[i]];
	}
	return hexArray;
}

function convertTextToTitleHex(inText)
{
	var hexArray = [];
	for (var i = 0; i < inText.length; ++i)
	{
		hexArray[i] = TEXT_MAPPING[inText[i]] + 0x20;
	}
	return hexArray;
}
...
function writeSymbolHash(rom, random)
{
	...
	//rom.set([0x5F,hash01,hash02,hash03,hash04,hash05,0x5F],TEXT_SEED_LOC+5);
	rom.set([hash01,0x5F,0x5F,hash02,0x5F,0x5F,hash03,0x5F,0x5F,hash04,0x5F,0x5F,hash05],TEXT_FLAGS_LOC+4);
}
...
function writeCredits(rom, random)
{
	...
	rom.set(
		[
			0x06, 0x41, 0x44, 0x44, 0x49, 0x54, 0x49, 0x4F, 0x4E, 0x41, 0x4C, 0x20, 0x44, 0x45, 0x56, 0x00, //ADDITIONAL DEV
			0x09, 0x42, 0x49, 0x4E, 0x41, 0x52, 0x59, 0x53, 0x50, 0x4C, 0x49, 0x54, 0x00, //BINARYSPLIT
			0x0C, 0x43, 0x4C, 0x4F, 0x55, 0x44, 0x59, 0x53, 0x48, 0x4F, 0x45, //CLOUDYSHOE
			0x00, 0x12, 0x42, 0x41,	0x53, 0x45, 0x44, 0x20, 0x4F, 0x4E, 0x20, 0x54, 0x48, 0x45, //BASED ON THE
			0x00, 0x15, 0x53, 0x4D, 0x57, 0x20,	0x52, 0x41, 0x4E, 0x44, 0x4F, 0x4D, 0x49, 0x5A, 0x45, 0x52, //SMW RANDOMIZED
			0x00, 0x18, 0x42, 0x59, 0x20, 0x41,	0x55, 0x54, 0x48, 0x4F, 0x52, 0x42, 0x4C, 0x55, 0x45, 0x53, //BY AUTHORBLUES
			0x00, 0x00, 0x00, 0x00, 0x00
		],
		0x01D4F0);

	setCreditNames(rom, random, 0x01D590);
...
function setCreditNames(rom, random, startAddress)
{
	var creditNames = ["CLOUDYSHOE", "FENYX4", "SATHDRESH", "SYNTHPOPISBACK", "ZOGGINS"];
	var finalCreditsString = [0x05, 0x54, 0x45, 0x53, 0x54, 0x45, 0x52, 0x53]; //TESTERS

	var hexNames = [];
	for(var i = 0; i < creditNames.length; ++i)
	{
		hexNames[i] = convertTextToTitleHex(creditNames[i]);
	}
	hexNames.shuffle(random);

	for(var i = 0; i < hexNames.length; ++i)
	{
		hexNames[i].unshift(0x00, 0x09+(0x03*i));
		finalCreditsString = finalCreditsString.concat(hexNames[i]);
	}

	finalCreditsString.push(0x00, 0x00);

	for(var i = 0; i < finalCreditsString.length; ++i)
	{
		rom[startAddress+i] = finalCreditsString[i];
	}
}
...
function fixText(rom, itemsRandomized)
{
	...
	//Moonstone names. Must end with 0x00
	//Additional parameter set to true is to use 0x00 instead of 0x5F (space) to clear area in the rom
	writeTextToAddress(rom, 0x0D044, 10, "M.Honesty", true); //Moonstone of Honesty
	rom.set([0x44, 0xD0], 0xDF7F); //Set item name pointer for Moonstone of Honesty
	writeTextToAddress(rom, 0x0D04E, 13, "M.Compassion", true); //Moonstone of Compassion
	rom.set([0x4E, 0xD0], 0xDF81); //Set item name pointer for Moonstone of Compassion
	writeTextToAddress(rom, 0x0D05B, 8, "M.Valor", true); //Moonstone of Valor
	rom.set([0x5B, 0xD0], 0xDF83); //Set item name pointer for Moonstone of Valor
	writeTextToAddress(rom, 0x0D063, 10, "M.Justice", true); //Moonstone of Justice
	rom.set([0x63, 0xD0], 0xDF85); //Set item name pointer for Moonstone of Justice
	writeTextToAddress(rom, 0x0D06D, 12, "M.Sacrifice", true); //Moonstone of Sacrifice
	rom.set([0x6D, 0xD0], 0xDF87); //Set item name pointer for Moonstone of Sacrifice
	writeTextToAddress(rom, 0x0D079, 8, "M.Honor", true); //Moonstone of Honor
	rom.set([0x79, 0xD0], 0xDF89); //Set item name pointer for Moonstone of Honor
	writeTextToAddress(rom, 0x0D081, 9, "M.Spirit", true); //Moonstone of Spirituality
	rom.set([0x81, 0xD0], 0xDF8B); //Set item name pointer for Moonstone of Spirituality
	writeTextToAddress(rom, 0x0D08A, 11, "M.Humility", true); //Moonstone of Humility
...
function changeNomadText(rom)
{
	var lzwData = decompressDataFromLZW(rom, 0x62E00);
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x85B); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0xAC6); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44, 0x53, 0x5F], 0xAC6); //change text to say nomads
	lzwData.set([0x2E, 0x4F, 0x4D, 0x41, 0x44, 0x53, 0x5F], 0xB18); //change text to say nomads
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0xDC9); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x12C2); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x14E2); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x1E08); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x2880); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x2B29); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x2B9B); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x2E73); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x2F4F); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x30C0); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x3EDE); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x3F8C); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x4147); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x43A2); //change text to say nomad
	lzwData.set([0x4E, 0x4F, 0x4D, 0x41, 0x44], 0x44C9); //change text to say nomad
	...
	if(inHintLocations.length > 0 && inHints.length > 0)
	{
		...
		for(var i = 0; i < inHintLocations.length; ++i)
		{
			if(hintText.length < inHintLocations[i].size)
			{
				...
				else if(inHintLocations[i].type == "dialog")
				{
...
function addSmithHint(rom, hintText)
{
	...
	lzwData.set([0xA8], 0x2F); //shift pointer for clue text

	//adjust IMPORTANT text to inject CLUE text que
	lzwData.set([0xFE, 0x22, 0x5F, 
	0x46, 0x4F, 0x52, 0x5F, 0x59, 0x4F, 0x55, 0x01, 0xF3, 0x39, 0x4F, 0x55, 0x07, 0x52, 0x45,
	0x5F, 0x54, 0x48, 0x45, 0x5F, 0x37, 0x41, 0x52, 0x52, 0x49, 0x4F, 0x52, 0x5F, 0x4F, 0x46, 0x5F,
	0x24, 0x45, 0x53, 0x54, 0x49, 0x4E, 0x59, 0x0C, 0x5F, 0x52, 0x49, 0x47, 0x48, 0x54, 0x1F, 0xF3,
	0x37, 0x45, 0x4C, 0x4C, 0x0C, 0x5F, 0x57, 0x48, 0x45, 0x4E, 0x5F, 0x59, 0x4F, 0x55, 0x5F, 0x47,
	0x4F, 0x5F, 0x54, 0x4F, 0x5F, 0x52, 0x45, 0x53, 0x43, 0x55, 0x45, 0x5F, 0xFE, 0x00, 0x5F, 0x46,
	0x52, 0x4F, 0x4D, 0x5F, 0x54, 0x48, 0x45, 0x5F, 0x55, 0x4E, 0x44, 0x45, 0x52, 0x57, 0x4F, 0x52,
